...
USE_SUCCESS_RATE = True # Used by post-processing files.
...
class Env(object):
    def __init__(self, game, norm_states, norm_rewards, params, add_t_with_horizon=None, clip_obs=None, clip_rew=None,
        ...
        self.retina_width = self.agent_view_size
        ...
        # --------------------------------------------------------------------------------------------------------------
        # Params from Implementation matters
        # --------------------------------------------------------------------------------------------------------------
    ...
    def encode_flat(self, obs, last_action):
        ...
        for row in range(self.agent_view_size):
            for col in range(self.agent_view_size):
    ...
    def step(self, action):
        ...
        if done:
            env_info['done'] = (self.counter, self.total_true_reward)
        ret = self.assemble_current_observation(obs, action), reward, done, env_info
        return ret
    ...
    def heldout_test(self):
        ...
        if sr < SUCCESS_RATE_THRESHOLD:
            ...
            se = SUCCESS_RATE_THRESHOLD * self.num_local_steps / max(0.1 / max_episodes,
                                                                     mean_sr)  # Should be global steps
        else:
    ...
    def online_test(self):
        ...
        metrics.append(
            (self.sample_efficiency, "{:2.0f}".format(self.sample_efficiency), "Projected sample efficiency"))
