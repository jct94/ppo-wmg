...
from utils.util import find_save_path, settings_to_dict, save_path_use_prefix
...
class Worker(object):
    ...
    def create_output_dir(self):
        ...
        try:
            expe = spec.val('experiment')
            results_dir = os.path.join(results_dir, expe)
        except:
            pass
        ...
        self.output_dir = find_save_path(results_dir, id)
        self.save_model_path = os.path.join(self.output_dir, 'model.pth')
        with open(os.path.join(self.output_dir, 'config.json'), 'w') as fp:
            json.dump(settings_to_dict(spec.settings), fp)

        csv_path = self.output_dir + 'log.csv'

        self.csv_writer = csv.writer(open(csv_path, 'a', 1))
        self.header = ['Time', 'Steps', 'Episodes', 'Success Rate', 'Projected sample Efficiency']
    ...
    def create_results_output_file(self):
        ...
        results_dir = self.create_output_dir()
    ...
    def test_episodes(self):
        ...
        for episode_id in range(NUM_EPISODES_TO_TEST):
            ...
            if (num_episodes_tested % (NUM_EPISODES_TO_TEST / 10) == 0):
                self.output(
                    '{:4d} / {:5d}  =  {:5.1f}%'.format(
                        num_wins, num_episodes_tested, 100.0 * num_wins / num_episodes_tested)
                )
            mean_reward += final_reward / num_episodes_tested
        self.output("Time: {:3.1f} min".format((time.time() - start_time) / 60.))
        self.output("Success rate = {} / {} episodes = {:5.1f}%".format(
            num_wins, num_episodes_tested, 100.0 * num_wins / num_episodes_tested)
        )
        self.output(f"Mean reward: {mean_reward}")
    ...
    def render_agent(self):
        """
        Loads an agent and renders its behavior.
        """
        self.init_episode()
        self.init_turtle()
        self.environment.use_display = True
    ...
    def monitor(self, reward):
        ...
        if (self.step_num % REPORTING_INTERVAL) == 0:
            ...
            data = [elapsed_time, self.num_steps, self.num_episodes] + [metric_tuple[i][1] for i in
                                                                        range(len(metric_tuple))]
