...
class SpecReader(object):
    def __init__(self, runspec_path, args=None):
        if args is not None:
            # We can initialize with a dictionnary of values
            self.settings = {}
            self.set_vals(args)
    ...
    def set_vals(self, args):
        # add new settings (line = -1 if argument is not defined in the config file)
